...
async function bootstrap(){
  ...
  const url=window.APP_CONFIG.APPS_SCRIPT_URL;
  if(!url){ setStatus('bad','Backend não configurado','Cole a URL do Apps Script em config.js'); applyUrlModuleParam(); return; }
  try{
    const res=await fetch(url+'?action=bootstrap'); const out=await res.json();
    state.members=out.members || []; setStatus('ok','Ligado','Backend e Google Sheets disponíveis'); renderForm(); applyUrlModuleParam(); loadStats();
  }catch(e){ setStatus('bad','Sem ligação','Verifique a URL do Apps Script'); console.warn(e); applyUrlModuleParam(); }
}
...
function buildAccountConciliationTable(accounts){
  ...
  return `
    <table class="report-table">
      ...
      <tbody>
        ${rows || `<tr><td colspan="7">Sem contas para apresentar.</td></tr>`}
      </tbody>
      <tfoot>
        <tr>
          <td>TOTAL</td>
