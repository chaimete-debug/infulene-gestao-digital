...
function buildAccountsReportTable(accounts){
  ...
  const rows = sorted.map(item => `
    <tr>
      ...
      <td>${cleanHtml(item.code || item.key || '')}</td>
      <td class="money">${money(item.saldoAnterior)}</td>
      <td class="money">${money(item.entradas)}</td>
      <td class="money">${money(item.saidas)}</td>
      <td class="money">${money(item.saldoPeriodo)}</td>
  ...
  return `
    <table class="report-table">
      ...
      <tbody>
        ${rows || `<tr><td colspan="7">Sem contas para o período seleccionado.</td></tr>`}
      </tbody>
      <tfoot>
        <tr>
          <td>TOTAL</td>
...
async function loadOverview(force=false){
  try{
    ...
    $('#overviewSubtitle').textContent = `Actualizado às ${new Date(data.generatedAt||Date.now()).toLocaleTimeString('pt-MZ',{hour:'2-digit',minute:'2-digit'})} · ${simpleNum(o.cultos)} cultos · ${simpleNum(o.visitantes)} visitantes · ${simpleNum(o.movimentos)} movimentos`;
    ...
    $('#overviewMin').textContent='Min: '+simpleNum(min); $('#overviewMax').textContent='Máx: '+simpleNum(max);
...
async function loadApprovals(){
  ...
  const url = window.APP_CONFIG.APPS_SCRIPT_URL;
  if(!url){ tbody.innerHTML = '<tr><td colspan="7">Backend não configurado.</td></tr>'; return; }
  try{
    ...
    `).join('') : '<tr><td colspan="7">Sem lançamentos pendentes para o seu perfil.</td></tr>';
  }catch(e){
    console.error(e);
    tbody.innerHTML = '<tr><td colspan="7">'+cleanHtml(e.message)+'</td></tr>';
  }
}
