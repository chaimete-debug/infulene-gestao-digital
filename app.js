...
function renderDelegadosBase(church, calc, ex=0, supl=0){
  ...
  const totalCom = totalSem + supl;
  if(hero) hero.textContent = totalCom || '—';
  if(box){
    if(!church){
      box.innerHTML = '<p>Seleccione uma igreja local para calcular automaticamente os delegados efectivos.</p>';
    }else{
      box.innerHTML = `
        <p>Os números abaixo são preenchidos automaticamente com base nos dados oficiais da igreja seleccionada.</p>
        <p><strong>Regime aplicável:</strong> ${cleanHtml(calc.descricao)}</p>
  ...
  if(totals){
    totals.innerHTML = `
      <div><span>Total de delegados da igreja sem suplentes</span><strong>${totalSem}</strong></div>
      <div><span>Total de suplentes registados</span><strong>${supl}</strong></div>
      <div><span>Total geral com suplentes registados</span><strong>${totalCom}</strong></div>
    `;
  }
}
...
function makeRepeatRows(containerId, prefix, count, type){
  ...
  for(let i=0;i<n;i++){
    const num = i + 1;
    if(type === 'exofficio'){
      rows.push(`<div class="repeat-row" data-i="${i}"><div class="repeat-index">${num}</div><label>Nome do delegado ex officio<input data-field="nome" placeholder="Nome completo"></label><label>Contacto<input data-field="contacto" placeholder="84xxxxxxx"></label><label>Categoria<select data-field="categoria">${categoriaOptions()}</select></label></div>`);
    }else if(type === 'suplente'){
      rows.push(`<div class="repeat-row two" data-i="${i}"><div class="repeat-index">${num}</div><label>Nome do suplente<input data-field="nome" placeholder="Nome completo"></label><label>Contacto<input data-field="contacto" placeholder="84xxxxxxx"></label></div>`);
    }else{
      rows.push(`<div class="repeat-row two" data-i="${i}"><div class="repeat-index">${num}</div><label>Nome do delegado efectivo <span class="req">*</span><input data-field="nome" placeholder="Nome completo" required></label><label>Contacto<input data-field="contacto" placeholder="84xxxxxxx"></label></div>`);
    }
  }
