...
function renderField(f, repeatName='', repeatIndex=''){
  ...
  if(t==='select_multiple' || t==='select_multiple_from_file'){
    const hidden=document.createElement('input');
    hidden.type='hidden'; hidden.name=inputName; hidden.id=inputName; hidden.dataset.originalName=name; hidden.dataset.isMulti='1';
    if(t==='select_multiple'){ hidden.dataset.listName=listName(f.type); hidden.dataset.choiceFilter=f.choice_filter||''; }
    if(t==='select_multiple_from_file'){ hidden.dataset.fromFile='members'; }
...
function renderMultiOptions(hidden){
  const group = document.querySelector(`[data-multi-group="${CSS.escape(hidden.id)}"]`);
  if(!group) return;
  const selected = new Set(selectedMultiValues(hidden));
  const options = getMultiOptions(hidden);
  group.innerHTML = options.length ? options.map((o, idx)=>{
    const id = `${hidden.id}__multi_${idx}`;
    const checked = selected.has(String(o.name)) ? 'checked' : '';
    return `<label class="multi-check-option" for="${id}"><input id="${id}" type="checkbox" value="${cleanHtml(o.name)}" data-multi-for="${hidden.id}" ${checked}> <span>${cleanHtml(o.label || o.name)}</span></label>`;
