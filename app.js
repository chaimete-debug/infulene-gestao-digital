const XLSFORM_FIELDS = [
  ...
  },
  {
    ...
    "name": "pin_acesso",
    "label": "PIN de acesso",
    "hint": "Acesso substituído por credenciais de utilizador.",
    "required": "false",
    "calculation": "",
    "appearance": "password",
    "relevant": "false",
  ...
  },
  {
    ...
    "name": "registrado_por",
    "label": "Registado por",
    "hint": "Escreva livremente o nome da pessoa que fez o registo.",
    "required": "true",
    "calculation": "",
    "appearance": "",
    "relevant": "",
...
function renderForm(){
  ...
  actions.innerHTML='<span class="pill" id="modulePill">Seleccione o módulo</span><button id="submitBtn" type="submit" class="primary-btn">Submeter</button>';
  form.appendChild(actions);
  form.addEventListener('input', onInput);
  form.addEventListener('change', onInput);
  form.addEventListener('submit', submitForm);
  refreshVisibility(); updateProgress(); updateFormAccessMode();
}
...
function renderField(f, repeatName='', repeatIndex=''){
  ...
  const label=cleanHtml(name==='menu_preencher' && hasOnlyViewAccess() ? 'O que deseja visualizar?' : (f.label||name));
  ...
  if(t==='select_one' || t==='select_multiple'){ el.dataset.listName=listName(f.type); el.dataset.choiceFilter=f.choice_filter||''; }
  if(repeatName) el.dataset.repeatName=repeatName, el.dataset.repeatIndex=repeatIndex;
  if(t==='select_one' || t==='select_multiple') fillChoiceOptions(el, listName(f.type), f.choice_filter || '');
...
function currentViewAreas(){ return normaliseList((state.user || {}).viewAreas); }
function currentSubmitAreas(){ return normaliseList((state.user || {}).submitAreas); }
function currentAllowedModules(){ return normaliseList((state.user || {}).allowedModules); }

function moduleArea(module){ return MODULE_AREAS[String(module || '')] || ''; }

function userCanViewModule(module){
...
function userCanSubmitModule(module){
  ...
  const user = state.user || {};
  const area = moduleArea(module);
  const submitAreas = currentSubmitAreas();
  const allowed = currentAllowedModules();
  return allowed.includes('*') || allowed.includes(String(module)) || submitAreas.includes(area) || !!user.canConfig;
}

function userCanModule(module){
  return userCanSubmitModule(module);
}

function isReadOnlyMode(){
  const module = getVal('menu_preencher');
  return !!module && userCanViewModule(module) && !userCanSubmitModule(module);
}

function hasOnlyViewAccess(){
  return currentSubmitAreas().length === 0 && currentAllowedModules().filter(x=>x && x !== '*').length === 0;
}

function updateFormAccessMode(){
  const module = getVal('menu_preencher');
  const readOnly = isReadOnlyMode();
  const noModule = !module;

  const menuLabel = document.querySelector('[data-field="menu_preencher"] label');
  if(menuLabel) menuLabel.childNodes[0].textContent = (readOnly || hasOnlyViewAccess()) ? 'O que deseja visualizar?' : 'O que deseja preencher?';

  const submitBtn = $('#submitBtn');
  if(submitBtn){
    submitBtn.classList.toggle('hidden', readOnly || noModule || !userCanSubmitModule(module));
    submitBtn.disabled = readOnly || noModule || !userCanSubmitModule(module);
  }

  const resetBtn = $('#resetBtn');
  if(resetBtn) resetBtn.classList.toggle('hidden', readOnly || hasOnlyViewAccess());

  const pill = $('#modulePill');
  if(pill){
    if(noModule) pill.textContent = hasOnlyViewAccess() ? 'Seleccione a área para visualizar' : 'Seleccione o módulo';
    else if(readOnly) pill.textContent = (optionLabel('menu_preencher', module) || module) + ' · modo de consulta';
    else pill.textContent = optionLabel('menu_preencher', module) || module;
  }

  $$('[name]').forEach(el=>{
    const original = el.dataset.originalName || el.name;
    if(original === 'menu_preencher') { el.disabled = false; return; }
    if(readOnly){
      el.disabled = true;
  ...
  $$('.repeater .ghost-btn, .repeat-item .danger-btn').forEach(btn=>{
    btn.classList.toggle('hidden', readOnly);
    btn.disabled = readOnly;
  });

  let note = $('#readOnlyNotice');
  if(readOnly){
    if(!note){
      note = document.createElement('div');
      note.id = 'readOnlyNotice';
      note.className = 'permission-notice';
      const form = $('#dynamicForm');
      form.insertBefore(note, form.firstChild);
    }
    note.innerHTML = '<strong>Modo de consulta.</strong> Este utilizador pode visualizar esta área, mas não pode efectuar lançamentos nem alterar dados.';
  } else if(note){
    note.remove();
  }
}
...
function fillChoiceOptions(el, list, choiceFilter=''){
  ...
  const options = (CHOICES[list]||[])
    .filter(o => list !== 'menu_preencher' || userCanViewModule(o.name))
...
function onInput(e){
  ...
  updateDerived(); refreshChoiceFilters(); applyPulldataToVisibleInputs(); refreshVisibility(); updateProgress(); updateFormAccessMode();
}
...
function refreshVisibility(){
  ...
  updateFormAccessMode();
}
function updateProgress(){
  const visibleFields=$$('.field:not(.hidden) [name]').filter(el=>!el.closest('.hidden') && !el.disabled);
...
function collectPayload(){
  ...
  return { action:'submit', authToken: state.authToken, user: state.user, module, submittedAt:new Date().toISOString(), uuid: getVal('submission_uuid') || uuid(), data:{...state.values}, repeats:state.repeats, userAgent:navigator.userAgent };
}
async function submitForm(e){
  ...
  if(!state.authToken) return toast('Faça login antes de submeter.', 'error');
  if(!userCanSubmitModule(getVal('menu_preencher'))) return toast('Este utilizador tem acesso apenas de visualização nesta área.', 'error');
  const btn=$('#submitBtn'); btn.disabled=true; btn.textContent='A submeter...';
  try{
    ...
    toast(out.estado_aprovacao === 'PENDENTE' ? 'Registo submetido e enviado para aprovação.' : 'Registo submetido e aprovado automaticamente.'); resetForm(); loadStats(); loadApprovals();
...
async function bootstrap(){
  ...
  try{
    const res=await fetch(url+'?action=bootstrap&token='+encodeURIComponent(state.authToken)); const out=await res.json(); if(!out.ok) throw new Error(out.message || 'Sessão inválida');
...
async function loadStats(){
  ...
  try{
    const res=await fetch(url+'?action=stats&token='+encodeURIComponent(state.authToken)); const out=await res.json(); if(!out.ok) throw new Error(out.message || 'Sessão inválida');
...
function buildAccountsReportTable(accounts){
  const sorted = [...(accounts || [])].sort((a, b) => String(a.name || a.label || a.key || a.code || '').localeCompare(String(b.name || b.label || b.key || b.code || ''), 'pt'));
  const rows = sorted.map(item => `
    <tr>
      <td>${cleanHtml(item.name || item.label || item.key || '')}</td>
      <td class="money">${money(item.saldoAnterior)}</td>
      <td class="money">${money(item.entradas)}</td>
      <td class="money">${money(item.saidas)}</td>
      <td class="money">${money(item.saldoPeriodo)}</td>
  ...
  return `
    <table class="report-table">
      ...
      <tbody>
        ${rows || `<tr><td colspan="6">Sem contas para o período seleccionado.</td></tr>`}
      </tbody>
      <tfoot>
        <tr>
          <td>TOTAL</td>
...
function renderFinancialReport(report){
  ...
  c.innerHTML = `
    ...
    <div class="report-section-title">Conciliação e resumo por contas</div>
    ${buildAccountsReportTable(report.accounts || [])}

    <div class="report-section-title">Por Rubrica</div>
    ${buildReportTable(report.rubricas || [], 'Rubrica', resolveRubricaLabel)}

    <div class="report-section-title">Por Departamento</div>
...
async function loadFinancialReport(){
  ...
  const params = new URLSearchParams({ action:'report', token: state.authToken });
...
const DASH_CACHE = { loaded:false, data:null };
const DASH_VIEWS = ['overviewView','escalasView','cultosView','visitantesView','dizimosView'];
function simpleNum(n){ return new Intl.NumberFormat('pt-MZ',{maximumFractionDigits:0}).format(Number(n||0)); }
function moneyPlain(n){ return new Intl.NumberFormat('pt-MZ',{minimumFractionDigits:2,maximumFractionDigits:2}).format(Number(n||0))+' MZN'; }
function monthMatches(dateStr, month){ return !month || String(dateStr||'').slice(0,7) === month; }
function emptyRow(cols, text){ return `<tr><td colspan="${cols}" class="legacy-empty">${text}</td></tr>`; }
async function getAppData(force=false){
  if(DASH_CACHE.loaded && !force) return DASH_CACHE.data;
  const url=window.APP_CONFIG.APPS_SCRIPT_URL;
  if(!url) throw new Error('Backend não configurado.');
  const res=await fetch(url+'?action=appdata&token='+encodeURIComponent(state.authToken));
  const out=await res.json();
  if(!out.ok) throw new Error(out.message || 'Não foi possível carregar os dados.');
  DASH_CACHE.loaded=true; DASH_CACHE.data=out; return out;
}
function kpiCard(label, value){ return `<div class="legacy-kpi"><span>${cleanHtml(label)}</span><b>${cleanHtml(value)}</b></div>`; }
function renderMiniBars(container, items){
  const el=$(container); if(!el) return;
  const max=Math.max(1,...(items||[]).map(x=>Number(x.value||0)));
  el.innerHTML=(items||[]).length ? `<div class="mini-bars">${items.map(x=>`<div class="mini-bar-row"><span>${cleanHtml(x.label||'')}</span><span class="line"><i style="width:${Math.round((Number(x.value||0)/max)*100)}%"></i></span><b>${simpleNum(x.value)}</b></div>`).join('')}</div>` : '<div class="legacy-empty">Sem dados</div>';
}
async function loadOverview(force=false){
  try{
    const data=await getAppData(force); const o=data.overview || {};
    const overviewSubtitle = $('#overviewSubtitle');
    if (overviewSubtitle) overviewSubtitle.textContent = `Actualizado às ${new Date(data.generatedAt||Date.now()).toLocaleTimeString('pt-MZ',{hour:'2-digit',minute:'2-digit'})} · ${simpleNum(o.cultos)} cultos · ${simpleNum(o.visitantes)} visitantes · ${simpleNum(o.movimentos)} movimentos`;
    $('#overviewKpis').innerHTML = [
      kpiCard('Cultos realizados', simpleNum(o.cultos)),
      kpiCard('Total visitantes', simpleNum(o.visitantes)),
      kpiCard('Média participantes', simpleNum(o.mediaParticipantes)),
      kpiCard('Saldo geral (MZN)', moneyPlain(o.saldoGeral||0))
    ].join('');
    renderMiniBars('#overviewVisitantesTipo', o.visitantesPorTipo||[]);
    renderMiniBars('#overviewComoConheceram', o.comoConheceram||[]);
    const pres=o.presenca||[]; const max=Math.max(1,...pres.map(x=>Number(x.value||0))); const min=pres.length?Math.min(...pres.map(x=>Number(x.value||0))):0;
    $('#overviewPresenca').innerHTML=pres.length?pres.map(x=>`<span title="${cleanHtml(x.label)}: ${simpleNum(x.value)}" style="height:${Math.max(4,Math.round((Number(x.value||0)/max)*52))}px"></span>`).join(''):'<div class="legacy-empty">Sem dados</div>';
    const overviewMin = $('#overviewMin');
    const overviewMax = $('#overviewMax');
    if (overviewMin) overviewMin.textContent = 'Min: ' + simpleNum(min);
    if (overviewMax) overviewMax.textContent = 'Máx: ' + simpleNum(max);
    renderMiniBars('#overviewRubricas', o.topRubricas||[]);
  }catch(e){ toast(e.message,'error'); }
}
function fillSelectOptions(select, values){
  const el=$(select); if(!el) return; const current=el.value;
  el.innerHTML='<option value="">Todas</option>'+[...new Set((values||[]).filter(Boolean))].sort().map(v=>`<option value="${cleanHtml(v)}">${cleanHtml(v)}</option>`).join('');
  el.value=current;
}
async function loadEscalas(){
  try{
    const data=await getAppData(); const month=$('#escalaMes')?.value||''; const func=$('#escalaFuncao')?.value||'';
    const all=(data.escalas||[]); fillSelectOptions('#escalaFuncao', all.map(r=>r.funcao));
    const rows=all.filter(r=>monthMatches(r.data,month)).filter(r=>!func || r.funcao===func);
    $('#escalasTable tbody').innerHTML = rows.length ? rows.map(r=>`<tr><td>${cleanHtml(r.data)}</td><td>${cleanHtml(r.hora)}</td><td>${cleanHtml(r.tipo)}</td><td>${cleanHtml(r.especificacao)}</td><td>${cleanHtml(r.dia)}</td><td>${cleanHtml(r.funcao)}</td><td>${cleanHtml(r.nome)}</td><td>${cleanHtml(r.telefone)}</td></tr>`).join('') : emptyRow(8,'Sem escalas para o período seleccionado');
  }catch(e){ toast(e.message,'error'); }
}
async function loadCultos(){
  try{
    const data=await getAppData(); const month=$('#cultosMes')?.value||''; const rows=(data.cultos||[]).filter(r=>monthMatches(r.data,month));
    const totalPart=rows.reduce((s,r)=>s+Number(r.participantes||0),0), totalVisit=rows.reduce((s,r)=>s+Number(r.visitantes||0),0), bapt=rows.reduce((s,r)=>s+Number(r.baptismos||0),0), decis=rows.reduce((s,r)=>s+Number(r.decisoes||0),0);
    $('#cultosKpis').innerHTML=[kpiCard('Total participantes',simpleNum(totalPart)),kpiCard('Total visitantes',simpleNum(totalVisit)),kpiCard('Baptismos',simpleNum(bapt)),kpiCard('Decisões',simpleNum(decis))].join('');
    $('#cultosList').innerHTML=rows.length?rows.map(r=>`<article class="culto-card"><span class="culto-badge">${simpleNum(r.participantes)} PAX</span><h3>${cleanHtml(r.tipo||'Culto')}</h3><div class="culto-meta"><span>📅 ${cleanHtml(r.data)}</span><span>⏰ ${cleanHtml(r.hora)}</span><span>📍 ${cleanHtml(r.local||'')}</span></div><div class="culto-grid"><div><small>Visitantes</small><b>${simpleNum(r.visitantes)}</b></div><div><small>Baptismos</small><b>${simpleNum(r.baptismos)}</b></div><div><small>Decisões</small><b>${simpleNum(r.decisoes)}</b></div><div><small>Sta. Ceia</small><b>${cleanHtml(r.santaCeia||'Não')}</b></div><div><small>Dízimos</small><b>${moneyPlain(r.dizimos)}</b></div><div><small>Ofertas</small><b>${moneyPlain(r.ofertas)}</b></div><div><small>Total</small><b>${moneyPlain(Number(r.dizimos||0)+Number(r.ofertas||0))}</b></div></div></article>`).join(''):'<div class="legacy-card legacy-empty">Sem relatórios para o período seleccionado</div>';
  }catch(e){ toast(e.message,'error'); }
}
async function loadVisitantes(){
  try{
    const data=await getAppData(); const month=$('#visitantesMes')?.value||''; const primeira=$('#visitantesPrimeira')?.value||'';
    const rows=(data.visitantes||[]).filter(r=>monthMatches(r.data,month)).filter(r=>!primeira || String(r.primeiraVez||'').toLowerCase()===primeira);
    const total=rows.length, primeiraCount=rows.filter(r=>String(r.primeiraVez||'').toLowerCase()==='sim').length, contacto=rows.filter(r=>String(r.contacto||'').toLowerCase()==='sim').length, oracao=rows.filter(r=>String(r.pedidoOracao||'').trim()).length;
    $('#visitantesKpis').innerHTML=[kpiCard('Total',simpleNum(total)),kpiCard('1ª visita',simpleNum(primeiraCount)),kpiCard('Querem contacto',simpleNum(contacto)),kpiCard('Pedidos de oração',simpleNum(oracao))].join('');
    $('#visitantesTable tbody').innerHTML=rows.length?rows.map(r=>`<tr><td>${cleanHtml(r.data)}</td><td>${cleanHtml(r.nome)}</td><td>${cleanHtml(r.sexo)}</td><td>${cleanHtml(r.faixa)}</td><td>${cleanHtml(r.bairro)}</td><td>${cleanHtml(r.primeiraVez)}</td><td>${cleanHtml(r.comoConheceu)}</td><td>${cleanHtml(r.contacto)}</td><td>${cleanHtml(r.telefone)}</td></tr>`).join(''):emptyRow(9,'Sem visitantes para o período');
  }catch(e){ toast(e.message,'error'); }
}
async function loadDizimos(){
  try{
    const data=await getAppData(); const month=$('#dizimosMes')?.value||''; const rows=(data.dizimos||[]).filter(r=>monthMatches(r.data,month));
    const total=rows.reduce((s,r)=>s+Number(r.valorMzn||0),0); const names=new Set(rows.map(r=>r.nome).filter(Boolean));
    $('#dizimosKpis').innerHTML=[kpiCard('Total MZN',moneyPlain(total)),kpiCard('Nº dizimistas',simpleNum(names.size)),kpiCard('Média por dizimista',moneyPlain(names.size?total/names.size:0)),kpiCard('Sessões registadas',simpleNum(rows.length))].join('');
    $('#dizimosTable tbody').innerHTML=rows.length?rows.map(r=>`<tr><td>${cleanHtml(r.data)}</td><td>${cleanHtml(r.nome)}</td><td>${cleanHtml(r.modo)}</td><td>${cleanHtml(r.valorOrig)}</td><td>${cleanHtml(r.moeda)}</td><td>${moneyPlain(r.valorMzn)}</td><td>${cleanHtml(r.metodo)}</td><td>${cleanHtml(r.recibo)}</td></tr>`).join(''):emptyRow(8,'Sem dízimos para o período');
  }catch(e){ toast(e.message,'error'); }
}
function loadLegacyView(view){
  if(view==='overviewView') return loadOverview();
  if(view==='escalasView') return loadEscalas();
  if(view==='cultosView') return loadCultos();
  if(view==='visitantesView') return loadVisitantes();
  if(view==='dizimosView') return loadDizimos();
}
function refreshLegacyData(){ DASH_CACHE.loaded=false; return loadLegacyView($('.nav-btn.active')?.dataset.view || 'overviewView'); }

function currentApprovalModules(){
...
function defaultClientPermissions(role){
  ...
  if(r === 'LIDER' || r === 'LÍDER') return { viewAreas: ['FINANCAS'], submitAreas: ['FINANCAS'], approveAreas: [] };
...
function ensureClientPermissions(user){
  ...
  // Regra rígida: líderes de departamentos/ministérios só lançam Finanças.
  // Ignora permissões antigas que possam ter ficado na folha Utilizadores.
  if(role === 'LIDER' || role === 'LÍDER') {
    user.role = 'LIDER';
    user.viewAreas = ['FINANCAS'];
    user.submitAreas = ['FINANCAS'];
    user.approveAreas = [];
    user.allowedModules = 'registo_financeiro';
    user.viewModules = 'registo_financeiro';
    user.approvalModules = '';
    user.canForm = true;
    user.canDashboard = false;
    user.canReport = true;
    user.canApprove = false;
    user.canConfig = false;
    return user;
  }
...
function applyUserPermissions(){
  ...
  $('#currentUserRole').textContent = user.role || 'Perfil';
  const viewAreas = user.viewAreas || [];
  const role = String(user.role || '').toUpperCase();
  const isLeader = role === 'LIDER' || role === 'LÍDER';

  let permissions = {
    overviewView: !!user.canDashboard || viewAreas.length > 0,
    escalasView: viewAreas.includes('PLANO'),
    cultosView: viewAreas.includes('CULTOS'),
    visitantesView: viewAreas.includes('MEMBROS'),
    dizimosView: viewAreas.includes('FINANCAS'),
    formView: !!user.canForm || viewAreas.length > 0,
    dashboardView: !!user.canDashboard || viewAreas.length > 0,
    reportView: !!user.canReport || viewAreas.includes('FINANCAS'),
    approvalView: !!user.canApprove || currentApprovalModules().length > 0,
    helpView: !!user.canConfig
  };

  // Líder: apenas Financeiro e Novo registo financeiro.
  if(isLeader){
    permissions = {
      overviewView: false,
      escalasView: false,
      cultosView: false,
      visitantesView: false,
      dizimosView: false,
      formView: true,
      dashboardView: false,
      reportView: true,
  ...
  $$('.nav-btn').forEach(btn=>{
    const can = permissions[btn.dataset.view] !== false;
    btn.classList.toggle('hidden-by-permission', !can);
  });
  const active = $('.nav-btn.active');
  if(active && active.classList.contains('hidden-by-permission')){
    const first = $$('.nav-btn').find(b=>!b.classList.contains('hidden-by-permission'));
...
async function loadApprovals(){
  ...
  if(!table) return;
  const thead = $('#approvalTable thead');
  const tbody = $('#approvalTable tbody');
  thead.innerHTML = '<tr><th>Data</th><th>Módulo</th><th>Submetido por</th><th>Departamento</th><th>Descrição</th><th>Valor</th><th>Acção</th></tr>';
  tbody.innerHTML = '<tr><td colspan="6">A carregar...</td></tr>';
  const url = window.APP_CONFIG.APPS_SCRIPT_URL;
  if(!url){ tbody.innerHTML = '<tr><td colspan="6">Backend não configurado.</td></tr>'; return; }
  try{
    const res = await fetch(url + '?action=approvals&token=' + encodeURIComponent(state.authToken));
    const out = await res.json();
    if(!out.ok) throw new Error(out.message || 'Não foi possível carregar aprovações.');
    const items = out.items || [];
    tbody.innerHTML = items.length ? items.map(item => `
      <tr>
        <td>${item.submittedAt || ''}</td>
        <td>${item.moduleLabel || item.module || ''}</td>
        <td>${item.submetido_por_nome || ''}</td>
        <td>${item.departamento || ''}</td>
        <td>${cleanHtml(item.descricao || '')}</td>
        <td>${item.valor || ''}</td>
        <td class="approval-actions">
          <button type="button" class="primary-btn small-btn" data-approve="${item.uuid}" data-module="${item.module}" data-decision="APROVADO">Aprovar</button>
          <button type="button" class="danger-btn small-btn" data-approve="${item.uuid}" data-module="${item.module}" data-decision="REJEITADO">Rejeitar</button>
        </td>
      </tr>
    `).join('') : '<tr><td colspan="6">Sem lançamentos pendentes para o seu perfil.</td></tr>';
  }catch(e){
    console.error(e);
    tbody.innerHTML = '<tr><td colspan="6">'+cleanHtml(e.message)+'</td></tr>';
  }
}

async function decideApproval(uuid, module, decision){
  const url = window.APP_CONFIG.APPS_SCRIPT_URL;
  if(!url) return toast('Backend não configurado.', 'error');
  const notes = decision === 'REJEITADO' ? (prompt('Indique a razão da rejeição:') || '') : '';
  try{
    const res = await fetch(url, {
      method:'POST',
      headers:{'Content-Type':'text/plain;charset=utf-8'},
      body: JSON.stringify({ action:'approve', authToken: state.authToken, uuid, module, decision, notes })
    });
    const out = await res.json();
    if(!out.ok) throw new Error(out.message || 'Não foi possível concluir a aprovação.');
    toast(decision === 'APROVADO' ? 'Lançamento aprovado.' : 'Lançamento rejeitado.');
    loadApprovals();
    loadStats();
  }catch(e){
    console.error(e);
...
function showApp(){
  ...
  const active = $('.nav-btn.active:not(.hidden-by-permission)') || $$('.nav-btn').find(b=>!b.classList.contains('hidden-by-permission'));
...
async function login(username, password){
  ...
  const out = await res.json();
  if(!out.ok){
    const msg = out.message || 'Falha no login.';
    if(String(msg).toLowerCase().includes('acção inválida') || String(msg).toLowerCase().includes('acao invalida')){
      throw new Error('O frontend já está com login, mas o Apps Script publicado ainda está numa versão antiga. Substitua o Code.gs pela versão nova e faça Deploy → Manage deployments → Edit → New version → Deploy.');
    }
    throw new Error(msg);
  }
...
$$('.nav-btn').forEach(btn => btn.addEventListener('click', () => {
  if(btn.classList.contains('hidden-by-permission')) return;
  $$('.nav-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  $$('.view').forEach(v => v.classList.remove('active'));
  $('#' + btn.dataset.view).classList.add('active');
  if (DASH_VIEWS.includes(btn.dataset.view)) loadLegacyView(btn.dataset.view);
  if (btn.dataset.view === 'dashboardView') loadStats();
  if (btn.dataset.view === 'reportView') loadFinancialReport();
  if (btn.dataset.view === 'approvalView') loadApprovals();
}));
...
$('#refreshOverviewBtn')?.addEventListener('click', refreshLegacyData);
$('#escalaMes')?.addEventListener('change', loadEscalas);
$('#escalaFuncao')?.addEventListener('change', loadEscalas);
$('#cultosMes')?.addEventListener('change', loadCultos);
$('#visitantesMes')?.addEventListener('change', loadVisitantes);
$('#visitantesPrimeira')?.addEventListener('change', loadVisitantes);
$('#dizimosMes')?.addEventListener('change', loadDizimos);
$('#approvalTable')?.addEventListener('click', e=>{
  const btn = e.target.closest('[data-approve]');
  if(!btn) return;
  decideApproval(btn.dataset.approve, btn.dataset.module, btn.dataset.decision);
});
initAuth();
