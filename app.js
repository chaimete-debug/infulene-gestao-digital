...
function userCanModule(module){
  ...
  const allowed = String(user.allowedModules || '*').split(',').map(x=>x.trim()).filter(Boolean);
  return allowed.includes('*') || allowed.includes(String(module || ''));
}
...
function fillChoiceOptions(el, list, choiceFilter=''){
  ...
  const options = (CHOICES[list]||[])
    .filter(o => list !== 'menu_preencher' || userCanModule(o.name))
...
async function submitForm(e){
  ...
  if(!userCanModule(getVal('menu_preencher'))) return toast('Este utilizador não tem permissão para submeter este módulo.', 'error');
...
function currentAllowedModules(){
  return String((state.user || {}).allowedModules || '').split(',').map(x=>x.trim()).filter(Boolean);
}
